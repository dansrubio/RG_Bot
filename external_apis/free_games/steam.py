"""
Fuente: Steam
Solo GamerPower como fuente de giveaways activos.
"""
...
URL_GAMERPOWER = "https://www.gamerpower.com/api/giveaways?platform=steam&type=game"
...
async def fetch(session: aiohttp.ClientSession) -> list[dict]:
    """Retorna giveaways activos de Steam via GamerPower"""
    try:
        async with session.get(URL_GAMERPOWER, timeout=aiohttp.ClientTimeout(total=12)) as r:
            r.raise_for_status()
            data = await r.json(content_type=None)
    except Exception as e:
        logger.error(f"[Steam/GamerPower] Error en request: {e}")
        return []
    ...
    juegos = []
    for item in data:
        if item.get("status", "").lower() != "active":  # ignorar giveaways inactivos
            continue
        juegos.append({
            "id":                f"steam_gp_{item.get('id', '')}",
